...
| `island.ts` | Программа на Ко Ронге |
| `leaders.ts` | Ведущие |
...
npm run build
npm run preview &
node scripts/screenshots.mjs      # скриншоты 375/390/430/768/1440 + проверки
```

`scripts/screenshots.mjs` заодно проверяет горизонтальную прокрутку, битые
якорные ссылки, `alt` у изображений, работу аккордеонов с клавиатуры и
ошибки в консоли. Скриншоты складываются в `.tmp/shots/`.
