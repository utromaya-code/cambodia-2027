...
export const hero = {
  kicker: 'Cambodia',
  title: 'Дорога на юг',
  lead: '11 дней в Камбодже. 10 дней маршрута от Ангкора до моря: красные дороги, деревни, джунгли, Бокор и побережье.',
  dates: trip.dates.humanShort,
  note: 'Можно ехать на байке или пройти тот же маршрут вместе с нами на микроавтобусе.',
  cta: 'Оставить заявку',
  /** Вторичное действие. Формулировку «обсудить участие» заказчик снял. */
  ctaSecondary: 'Посмотреть маршрут',
  /**
...
/**
 * Полоса доверия сразу под первым экраном (пункт 4.2 ТЗ).
 *
 * Правило блока — «публиковать только подтверждённые факты»: половина
 * пунктов из ТЗ (местный мото-гид, машина сопровождения, отдельная перевозка
 * багажа, размер группы, уровень отелей) организатором не подтверждена,
 * поэтому лежит здесь как TODO_CONTENT и до подтверждения не выводится.
 * Компонент фильтрует список сам и прячет всю полосу, если подтверждённых
 * пунктов осталось меньше трёх: строка из одного факта выглядит не как
 * аргумент, а как недоделка.
 *
 * Значения ниже — те же данные, что уже показаны на странице:
 * четыре карточки в Leaders, два формата в Pricing, модель из bike.ts
...
export const trustStrip: readonly Pending<string>[] = [
  ...
  TODO_CONTENT, // местный лицензированный мото-гид
  TODO_CONTENT, // машина сопровождения
  TODO_CONTENT, // багаж едет отдельно
  TODO_CONTENT, // небольшая группа до N участников
  TODO_CONTENT, // комфортные отели по маршруту
];
