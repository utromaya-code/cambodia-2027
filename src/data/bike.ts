...
export const bike = {
  ...
  image: 'bike-ftr-ref',
  /*
   * Пункт 4.8 ТЗ запрещает показывать чужой мотоцикл как парк проката.
   * Это снимок именно Honda FTR 223 — той самой модели, — но не из парка
   * камбоджийского партнёра. Подпись под кадром говорит это прямо, а как
   * только придут фотографии реальных байков, кадр и подпись уходят.
   */
 ...
 *
 * Кадры — обе названные модели: 411 и 450. Раньше здесь стояли снимки
 * бутанской экспедиции; они честные, но на них байки среди людей и гор,
 * а человеку, выбирающему доплату, нужно увидеть саму технику. Бутанские
 * кадры никуда не делись — они в блоке «Мы уже так ездили».
 */
export const bikeUpgrade = {
  ...
  note: 'Снимки моделей, а не байков из проката: слева 411, справа 450. На таких же мы шли по Бутану — те кадры в блоке «Мы уже так ездили».',
} as const;
